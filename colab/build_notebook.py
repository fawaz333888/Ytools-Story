...
CELLS: list[dict] = [
    {
        ...
        "source": [
            ...
            "Upload footage di **Cell 7** (setelah story), atur opsi di **Cell 5**, generate story+judul di **Cell 6**, render di **Cell 8**, lalu lihat hasil di **Cell 9-10**.\n",
        ],
    },
    {
        "type": "code",
    ...
    },
    {
        ...
        "source": [
            ...
            "#@markdown *`badge` = kotak gelap; `plain` = teks polos; `logo` = upload gambar logo (cell 8).*\n",
            ...
            "#@markdown *Thumbnail + judul di atas. Upload gambar thumbnail di cell 8.*\n",
            ...
            "#@markdown *Nama file output (disimpan ke Drive di cell 10).*\n",
    ...
    },
    {
        "type": "code",
        "source": [
            "#@title 6. Generate story + title (wajib sebelum render)\n",
            "import datetime\n",
            "\n",
            "save_story_to_drive = True #@param {type:'boolean'}\n",
            "#@markdown *Simpan story.txt + title.txt ke Drive (folder tanggal). Direuse sama cell 10 buat simpan video.*\n",
            "\n",
            "from ytools.story import generator as story_gen\n",
            "\n",
            ...
            "    STORY = open(SCRIPT, encoding='utf-8').read()\n",
            "else:\n",
            ...
            "        api_key_env=cfg.get('story.api_key_env'),\n",
            "        base_url=llm_base_url or None,\n",
            "        seed=seed,\n",
            "    )\n",
            "    assert STORY, 'LLM mengembalikan story kosong'\n",
            "    SCRIPT = '/content/generated_story.txt'\n",
            "    open(SCRIPT, 'w', encoding='utf-8').write(STORY)\n",
            "\n",
            "# title pakai LLM kalau kosong; manual tetap bisa kalau ada key\n",
            "if not TITLE and (card_enabled or os.environ.get('OPENAI_API_KEY') or os.environ.get('ANTHROPIC_API_KEY')):\n",
            "    _title_provider = provider if provider in ('openai', 'anthropic') else (\n",
            "        'openai' if os.environ.get('OPENAI_API_KEY') else 'anthropic'\n",
            "    )\n",
            "    _key_env = cfg.get('story.api_key_env') if _title_provider == provider else (\n",
            "        'OPENAI_API_KEY' if _title_provider == 'openai' else 'ANTHROPIC_API_KEY'\n",
            "    )\n",
            "    if not os.environ.get(_key_env):\n",
            "        raise RuntimeError(\n",
            "            f'card_title kosong dan key {_key_env} tidak ada. Isi card_title di cell 5 atau set API key.'\n",
            "        )\n",
            ...
            "    )\n",
            "\n",
            "cfg.set('overlays.card.title', TITLE or None)\n",
            "\n",
            "if save_story_to_drive:\n",
            "    run_dir = os.path.join(DRIVE_ROOT, datetime.date.today().isoformat())\n",
            "    os.makedirs(run_dir, exist_ok=True)\n",
            "    open(os.path.join(run_dir, 'story.txt'), 'w', encoding='utf-8').write(STORY)\n",
            "    open(os.path.join(run_dir, 'title.txt'), 'w', encoding='utf-8').write(TITLE)\n",
            "    print('story+title saved to', run_dir)\n",
            "\n",
            "print('TITLE:', TITLE or '(kosong - card gak akan ada judul)')\n",
            "print('STORY preview:', STORY[:400].replace(chr(10), ' '))\n",
        ],
    },
    {
        "type": "code",
        "source": [
            "#@title 7. Upload footage video (minimal 30 detik)\n",
            "from google.colab import files\n",
            "import shutil, os\n",
            "\n",
            "FOOTAGE = 'footage.mp4'\n",
    ...
    },
    {
        "type": "code",
        "source": [
            "#@title 8. RUN pipeline (render-only)\n",
            "from ytools.pipeline import Pipeline\n",
            "\n",
            "workdir = '/content/ytools_work/run'\n",
            "pipe = Pipeline(workdir, cfg, ff=ff)\n",
            "\n",
            "# story+title sudah dibuat di cell 6; load ulang, jangan generate di sini\n",
            "assert 'STORY' in globals() and STORY, 'jalankan cell 6 (generate story+title) dulu'\n",
            "script_arg = SCRIPT\n",
            "\n",
    ...
    },
    {
        "type": "code",
        "source": [
            "#@title 10. Simpan video ke Drive (video only)\n",
            "import shutil\n",
            "\n",
            "# reuse folder dari cell 6 kalau ada (story+title sudah disimpan sana)\n",
            "run_dir = run_dir if 'run_dir' in globals() else os.path.join(DRIVE_ROOT, datetime.date.today().isoformat())\n",
            "os.makedirs(run_dir, exist_ok=True)\n",
            "\n",
    ...
    },
    {
        ...
        "source": [
            "## Catatan\n",
            "\n",
            "- **Session Colab free** maksimal ~12 jam, idle disconnect ~90 menit. Hasil otomatis tersimpan ke Drive (cell 6 story+title, cell 10 video) — aman berhenti kapan saja.\n",
            "- **Reuse**: cell 8 menggunakan cache — rerun cepat kalau footage/particles/watermark sudah ada. Cache key mencakup parameter render (style, density, size, font, dll), jadi ubah parameter itu = render ulang otomatis. **Opacity** (particle, watermark, spectrum) diterapkan saat compositing, jadi ubah opacity = tetap pakai cache tanpa render ulang. **Tombol subscribe**: teks di-cache (PNG), waktu muncul + animasi goyangan di compositing — ubah `subscribe_appear` = no re-render.\n",
            "- **Story+title**: cell 6 generate/upload sekali, hasil diload ulang di cell 8. LLM story+title tergantung `seed` — isi seed yang sama untuk hasil reproducible.\n",
