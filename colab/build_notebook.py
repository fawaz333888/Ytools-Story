...
CELLS: list[dict] = [
    {
        ...
        "source": [
            ...
            "Upload footage di **Cell 4**, atur opsi di **Cell 5**, generate story+judul di **Cell 6**, render di **Cell 7**, lalu lihat hasil di **Cell 8-9**.\n",
        ],
    },
    {
        "type": "code",
    ...
    },
    {
        ...
        "source": [
            ...
            "#@markdown *Simpan story.txt + title.txt ke Drive (folder tanggal). Direuse sama cell 9 buat simpan video.*\n",
            "\n",
    ...
    },
    {
        "type": "code",
        "source": [
            "#@title 7. RUN pipeline (render-only)\n",
    ...
    },
    {
        "type": "code",
        "source": [
            "#@title 9. Simpan video ke Drive (video only)\n",
            "import shutil\n",
            "\n",
    ...
    },
    {
        ...
        "source": [
            "## Catatan\n",
            "\n",
            "- **Session Colab free** maksimal ~12 jam, idle disconnect ~90 menit. Hasil otomatis tersimpan ke Drive (cell 6 story+title, cell 9 video) — aman berhenti kapan saja.\n",
            "- **Reuse**: cell 7 menggunakan cache — rerun cepat kalau footage/particles/watermark sudah ada. Cache key mencakup parameter render (style, density, size, font, dll), jadi ubah parameter itu = render ulang otomatis. **Opacity** (particle, watermark, spectrum) diterapkan saat compositing, jadi ubah opacity = tetap pakai cache tanpa render ulang. **Tombol subscribe**: teks di-cache (PNG), waktu muncul + animasi goyangan di compositing — ubah `subscribe_appear` = no re-render.\n",
            "- **Story+title**: cell 6 generate/upload sekali, hasil diload ulang di cell 7. LLM story+title tergantung `seed` — isi seed yang sama untuk hasil reproducible.\n",
